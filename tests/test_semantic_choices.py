...
class SemanticChoiceTests(unittest.TestCase):
    ...
    def test_option_a_completes_objective(self):
        self.assertEqual(self.choose(self.choice((ChoiceConsequence("YES", ConsequenceKnowledge.KNOWN, {"complete": True}),))), "YES")

    def test_option_b_completes_objective(self):
        self.assertEqual(self.choose(self.choice((ChoiceConsequence("NO", ConsequenceKnowledge.KNOWN, {"complete": True}),))), "NO")
    ...
    def test_selected_desired_option_is_confirmed_without_movement(self):
        choice = self.choice((ChoiceConsequence("YES", ConsequenceKnowledge.KNOWN, {"complete": True}),), selected="YES")
        self.assertEqual(self.choose(choice), "YES")

    def test_unavailable_desired_option_is_not_fabricated(self):
        choice = self.choice((ChoiceConsequence("NO", ConsequenceKnowledge.KNOWN, {"complete": False}),), options=("NO",))
        self.assertIsNone(self.choose(choice))
