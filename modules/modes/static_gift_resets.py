...
class StaticGiftResetsMode(BotMode):
    ...
    def run(self) -> Generator:
        ...
        if encounter is None:
            raise BotModeError(
                "You are not facing the NPC or tile that gives you the gift encounter."
            )
        ...
        if encounter[2] == "Wynaut":
            ...
            if save_data.get_event_flag("RECEIVED_LAVARIDGE_EGG"):
                raise BotModeError(
                    "You have already received the Wynaut egg in your saved game."
                )
        if (
            context.rom.is_emerald
            and encounter[2] in ["Wynaut", "Togepi"]
            and not any(
                pokemon.ability.name in ["Flame Body", "Magma Armor",]
                for pokemon in save_data.get_party()
            )
        ):
            console.print(
                "[bold yellow]WARNING: None of your Pokémon has the Flame Body / Magma Armor ability.[/]"
            )
        ...
        if encounter[2] == "Togepi":
            if save_data.get_event_flag("GOT_TOGEPI_EGG"):
                raise BotModeError(
                    "You have already received the Togepi egg in your saved game."
                )
            assert_registered_item(
                ["Bicycle"],
        ...
        if encounter[2] in ("Hoenn Fossils", "Kanto Fossils"):
            ...
            if save_data.get_event_var(var_name) == 1:
                if get_event_var(var_name) == 1:
                    ...
                    raise BotModeError(
                        "You need to save the game before using this mode."
                    )
        ...
        while context.bot_mode != "Manual":
            ...
            if context.rom.is_frlg:
                ...
                yield from wait_until_task_is_not_active(
                    "Task_DrawFieldMessageBox", "B"
                )
            ...
            if context.rom.is_frlg and encounter[2] not in ["Togepi"]:
                if encounter[2] in ["Hitmonchan", "Hitmonlee"]:
                    yield from wait_until_event_flag_is_true(
                        "GOT_HITMON_FROM_DOJO", "B"
                    )
                yield from wait_for_task_to_start_and_finish(
                    "Task_YesNoMenu_HandleInput", "B"
                )
            ...
            if context.rom.is_emerald and encounter[2] not in ["Wynaut"]:
                yield from wait_for_task_to_start_and_finish(
                    "Task_DrawFieldMessage", "B"
                )
                yield from wait_for_task_to_start_and_finish(
                    "Task_HandleYesNoInput", "B"
                )
            ...
            def hatch_egg() -> Generator:
                if encounter[2] == "Wynaut":
                    ...
                    point_b = get_map_data(
                        MapFRLG.FIVE_ISLAND_WATER_LABYRINTH, (17, 13)
                    )
                else:
                    raise BotModeError("Unknown encounter type")

                yield from navigate_to(
                    point_a.map_group_and_number, point_a.local_position
                )
            ...
            if encounter[2] in ["Wynaut", "Togepi"]:
                ...
                yield from PokemonPartyMenuNavigator(
                    get_party_size() - 1, "summary"
                ).step()
